...
class TorrentStreamServer(private val saveDir: File) {
    ...
    private fun handleClient(client: Socket) {
        try {
            ...
            if (fileLength <= 0) { sendError(client, 500, "Empty file"); return }
            ...
            if (startOffset >= safeNow) {
                ...
                if (pieceChecker?.invoke(startPiece) != true || (startPiece != endPiece && pieceChecker?.invoke(endPiece) != true)) {
                    ...
                    if (!available) { sendError(client, 503, "Not yet available"); return }
                }
            }
            ...
            try {
                ...
                while (remaining > 0 && running) {
                    ...
                    if (canRead <= 0) {
                        if (System.nanoTime() - waitStart > 30_000_000_000L) break
                        try { Thread.sleep(100) } catch (_: InterruptedException) { break }
                        continue
                    }
